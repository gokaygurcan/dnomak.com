<template lang="pug">
  div
    c-header
    .xl-mt24.xl-mx16(v-if='humans.length || human')
      .container
            ...
            .xl-mb16.xl-tal.xl-co-black-500.xl-br8.xl-ba-white.xl-bw2.xl-bo-gray-200.xl-p24.lg-p16
              .wrap.xl-auto.xl-gutter-16.xl-middle.xl-pr
                .col.sm-hidden(v-if='human.username')
                  .avatar--human.xl-br8(:style="{ 'background-image': 'url(/static/img/youtube/' + human.username + '.jpg)' }")
                .col
                  .youtube
...
<script>
  ...
  export default {
    ...
    created() {
      this.allHumans();
      this.allTimes(this.$route.params.username);
      this.allAccounts(this.$route.params.username);
    ...
    watch: {
      humans() {
        this.human = this.humanByUsername(this.$route.params.username);
      },
    },
    computed: {
      ...mapGetters('humans', [
